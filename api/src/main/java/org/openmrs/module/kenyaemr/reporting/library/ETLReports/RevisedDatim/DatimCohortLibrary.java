   ...
   //Number restarted Treatment during the reporting period
    ...
    public CohortDefinition currentSuppVLResultByTestType(String testType) {
        ...
        String sqlQuery = "select a.patient_id from (select\n" +
                ...
                "   from (select x.patient_id as patient_id,x.visit_date as visit_date,x.lab_test as lab_test,x.date_test_requested, x.test_result as test_result,urgency as urgency\n" +
                ...
                "   group by patient_id\n" +
                "   having mid(max(concat(visit_date,lab_test)),11) in (1305,856) and max(visit_date) between\n" +
                "   date_sub(:endDate , interval 12 MONTH) and date(:endDate) and urgency = "+testType+" and (vl_result < 1000 or vl_result='LDL'))a;";
    ...
    public CohortDefinition kpByKPType(KPTypeDataDefinition kpType) {
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery = "select kp.patient_id from (select fup.patient_id,max(fup.visit_date) as ltst_visit_date,mid(max(concat(fup.visit_date,fup.key_population_type)),11) as kp_type from kenyaemr_etl.etl_patient_hiv_followup fup group by fup.patient_id\n" +
                "having kp_type = "+kpType.getKpTypeConcept()+")kp;";
    ...
    /**
     * Patiients started ART more than 3 months ago
     * @return
     */
    public CohortDefinition startedArtOverThreeMonths() {
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery = "select e.patient_id from (select e.patient_id,min(date(e.date_started)) as date_started  from kenyaemr_etl.etl_drug_event e where e.program ='HIV'\n" +
                "group by e.patient_id having timestampdiff(MONTH,date_started,DATE(:endDate)) > 3)e;";
        cd.setName("startedArtOverThreeMonths");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.setDescription("startedArtOverThreeMonths");
    ...
    public CohortDefinition txpvlsDenominator() {
        ...
        cd.addSearch("startedArtOverThreeMonths", ReportUtils.map(startedArtOverThreeMonths(), "startDate=${startDate},endDate=${endDate}"));
        cd.addSearch("currentVLResultLast12Months", ReportUtils.map(currentVLResultLast12Months(), "startDate=${startDate},endDate=${endDate}"));
        cd.setCompositionString("txcurr AND startedArtOverThreeMonths");
        return cd;
    }
     ...
     * @return
     */
    public CohortDefinition currentlyOnARTKP(KPTypeDataDefinition kpType) {
        CompositionCohortDefinition cd = new CompositionCohortDefinition();
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.addSearch("txcurr",
     ...
     * @return
     */
    public CohortDefinition currentlyOnARTMMD(DurationToNextAppointmentDataDefinition duration) {
        CompositionCohortDefinition cd = new CompositionCohortDefinition();
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.addSearch("txcurr",
                ReportUtils.map(currentlyOnArt(), "startDate=${startDate},endDate=${endDate}"));
        cd.addSearch("monthsToTCA",
                ReportUtils.map(monthsToTCA(duration), "startDate=${startDate},endDate=${endDate}"));
        cd.setCompositionString("txcurr AND monthsToTCA");
        return cd;
    }
    ...
    public CohortDefinition onARTSuppVLAgeSex(String testType) {
        ...
        cd.addSearch("startedArtOverThreeMonths", ReportUtils.map(startedArtOverThreeMonths(), "startDate=${startDate},endDate=${endDate}"));
        cd.setCompositionString("txcurr AND currentSuppVLResultByTestType AND startedArtOverThreeMonths");
        return cd;
    }
    ...
    public CohortDefinition onARTKpWithSuppVLLast12Months(String testType, KPTypeDataDefinition kpType) {
        CompositionCohortDefinition cd = new CompositionCohortDefinition();
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.addSearch("txcurr",
        ...
        cd.addSearch("currentSuppVLResultByTestType", ReportUtils.map(currentSuppVLResultByTestType(testType), "startDate=${startDate},endDate=${endDate}"));
        cd.addSearch("startedArtOverThreeMonths", ReportUtils.map(startedArtOverThreeMonths(), "startDate=${startDate},endDate=${endDate}"));
        cd.setCompositionString("txcurr AND keyPop AND currentSuppVLResultByTestType AND startedArtOverThreeMonths");
        return cd;
    }
    ...
    public CohortDefinition kpNewlyStartedART(KPTypeDataDefinition kpType) {
        CompositionCohortDefinition cd = new CompositionCohortDefinition();
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.addSearch("txcurr",
        ...
        cd.addSearch("keyPop",
                ReportUtils.map(kpByKPType(kpType), "startDate=${startDate},endDate=${endDate}"));
        cd.addSearch("newlyStartedArt", ReportUtils.map(startedOnART(), "startDate=${startDate},endDate=${endDate}"));
        cd.setCompositionString("keyPop AND newlyStartedArt");
        return cd;
    }

    /*Patients With VL result within last 12 months*/
    public CohortDefinition currentVLResultLast12Months() {

        String sqlQuery = "select vl.patient_id from (select\n" +
                "patient_id,\n" +
                "max(visit_date) as vl_date,\n" +
                "date_sub(:endDate , interval 12 MONTH) as ref_date,\n" +
                "if(mid(max(concat(visit_date,lab_test)),11) = 856, mid(max(concat(visit_date,test_result)),11), if(mid(max(concat(visit_date,lab_test)),11)=1305 and mid(max(concat(visit_date,test_result)),11) = 1302, \"LDL\",\"\")) as vl_result,\n" +
                "mid(max(concat(visit_date,urgency)),11) as urgency\n" +
                "from kenyaemr_etl.etl_laboratory_extract\n" +
                "group by patient_id\n" +
                "having mid(max(concat(visit_date,lab_test)),11) in (1305,856) and vl_date between\n" +
    ...
    public CohortDefinition txmlPatientByTXStopReason(TreatmentStopReasonDataDefinition reason) {
        CompositionCohortDefinition cd = new CompositionCohortDefinition();
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
    ...
    /**
     * TX_ML IIT KP patients
     * @return
     */
    public CohortDefinition txMLIITKp(KPTypeDataDefinition kpType, DurationToNextAppointmentDataDefinition months) {
        CompositionCohortDefinition cd = new CompositionCohortDefinition();
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.addSearch("kp", ReportUtils.map(kpByKPType(kpType), "startDate=${startDate},endDate=${endDate}"));
        cd.addSearch("txMLIIT", ReportUtils.map(txMLIIT(months), "startDate=${startDate},endDate=${endDate}"));
        cd.setCompositionString("txMLIIT AND kp");
        return cd;
    }
    /**
     ...
     * @return
     */
    public CohortDefinition txmlPatientByCauseOfDeath(TreatmentStopReasonDataDefinition causeOfDeath) {
        CompositionCohortDefinition cd = new CompositionCohortDefinition();
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
     ...
     * @return
     */
    public CohortDefinition txMLSpecificCauseOfDeath(TreatmentStopReasonDataDefinition specificCauseOfDeath) {
        CompositionCohortDefinition cd = new CompositionCohortDefinition();
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
     ...
     * @return
     */
    public CohortDefinition txmlKPPatientStopReason(KPTypeDataDefinition kpType,TreatmentStopReasonDataDefinition reason) {
        CompositionCohortDefinition cd = new CompositionCohortDefinition();
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.addSearch("kp",
    ...
    public CohortDefinition txMLIIT(DurationToNextAppointmentDataDefinition noOfMonths) {
    ...
    String sqlQuery = "select tx_ml.patient_id from (select t.patient_id,t.date_started,t.latest_vis_date,t.latest_tca from(\n" +
            ...
            "                                  (((date(latest_tca) < :endDate) and (date(latest_vis_date) < date(latest_tca))) ) and ((date(latest_tca) > date(date_discontinued) and date(latest_vis_date) > date(date_discontinued)) or disc_patient is null ) and datediff(:endDate, date(latest_tca)) between 1 and 90)\n" +
            "                       ) e on t.patient_id = e.patient_id) tx_ml\n" +
            "                      where (datediff(:endDate, date(tx_ml.latest_tca))>30) and datediff(tx_ml.latest_vis_date,tx_ml.date_started) "+noOfMonths.getDuration()+" group by tx_ml.patient_id;";
    SqlCohortDefinition cd = new SqlCohortDefinition();
    cd.setName("TX_ML_IIT");
    cd.setQuery(sqlQuery);
    cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
    cd.addParameter(new Parameter("endDate", "End Date", Date.class));
...
/*Patients by treatment stop reason*/
    public CohortDefinition patientByTXStopReason(TreatmentStopReasonDataDefinition reason) {

        String sqlQuery = "select patient_id from (select dt.patient_id from kenyaemr_etl.etl_ccc_defaulter_tracing dt where dt.is_final_trace =1267 and dt.true_status ="+reason.getTxStopReasonConcept()+" and date(dt.visit_date) between date(:startDate) and date(:endDate) group by dt.patient_id)dt\n" +
                "    union\n" +
                "    (select d.patient_id from kenyaemr_etl.etl_patient_program_discontinuation d where d.discontinuation_reason= "+reason.getTxStopReasonConcept()+" and d.visit_date between date(:startDate) and date(:endDate) group by d.patient_id);";
        SqlCohortDefinition cd = new SqlCohortDefinition();
    ...
    public CohortDefinition patientByTXStopDeathReason(TreatmentStopReasonDataDefinition reason) {

        String sqlQuery = "select patient_id from (select dt.patient_id from kenyaemr_etl.etl_ccc_defaulter_tracing dt where dt.is_final_trace =1267 and dt.cause_of_death ="+reason.getTxStopReasonConcept()+" and date(dt.visit_date) between date(:startDate) and date(:endDate) group by dt.patient_id)dt\n" +
                "    union\n" +
                "    (select d.patient_id from kenyaemr_etl.etl_patient_program_discontinuation d where d.death_reason = "+reason.getTxStopReasonConcept()+" and d.visit_date between date(:startDate) and date(:endDate) group by d.patient_id);";
        SqlCohortDefinition cd = new SqlCohortDefinition();
    ...
    public CohortDefinition patientByTXStopSpecificDeathReason(TreatmentStopReasonDataDefinition specificCauseOfDeath) {

        String sqlQuery = "select patient_id from (select dt.patient_id from kenyaemr_etl.etl_ccc_defaulter_tracing dt where dt.is_final_trace =1267 and dt.cause_of_death ="+specificCauseOfDeath.getTxStopReasonConcept()+" and date(dt.visit_date) between date(:startDate) and date(:endDate) group by dt.patient_id)dt\n" +
                "    union\n" +
                "    (select d.patient_id from kenyaemr_etl.etl_patient_program_discontinuation d where d.specific_death_cause ="+specificCauseOfDeath.getTxStopReasonConcept()+" and d.visit_date between date(:startDate) and date(:endDate) group by d.patient_id);";
        SqlCohortDefinition cd = new SqlCohortDefinition();
        cd.setName("treatmentStopReason");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
     ...
     * TX_CURR_MMD subquery
     * @return
     */
    public CohortDefinition monthsToTCA(DurationToNextAppointmentDataDefinition duration) {
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery="select f.patient_id from (select f.patient_id,timestampdiff(MONTH,max(f.visit_date),mid(max(concat(f.visit_date,f.next_appointment_date)),11))  months_tca from kenyaemr_etl.etl_patient_hiv_followup f\n" +
                "    group by f.patient_id having months_tca "+duration.getDuration()+")f;";
        cd.setName("MMD_SUB");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.setDescription("Months to tca");
        return cd;
    }
    ...
    public CohortDefinition prepCT() {
        String sqlQuery = "select p.patient_id\n" +
                ...
                "             e.first_enr_date         as date_first_enrolled,\n" +
                "             e.latest_enr_client_type as latest_enr_client_type,\n" +
                "             e.latest_enr_date as latest_enr_date,\n" +
                "             p.patient_id             as fup_patient,\n" +
                "             p.prep_visit_date             as fup_date,\n" +
                "             P.final_test_result as hiv_status,\n" +
                "             e.kp_type as kp_type,\n" +
                "             p.pregnant as pregnant,\n" +
                "             p.breastfeeding as breastfeeding\n" +
                "      from (select e.patient_id,\n" +
                "                   min(e.visit_date)                                         as first_enr_date,\n" +
                "                   max(e.visit_date)                                         as latest_enr_date,\n" +
                "                   mid(max(concat(e.visit_date, e.patient_type)), 11)        as latest_enr_client_type,\n" +
                "                   mid(max(concat(e.visit_date, e.kp_type)), 11) as kp_type\n" +
                "            from kenyaemr_etl.etl_prep_enrolment e\n" +
                "            group by e.patient_id) e\n" +
                "             left join (select f.patient_id,\n" +
                "                               f.visit_date as prep_visit_date,\n" +
                "                               t.visit_date as test_date,\n" +
                "                               f.breastfeeding,\n" +
                "                               f.pregnant,\n" +
                "                               f.prep_contraindication,\n" +
                "                               t.final_test_result\n" +
                "                        from (select f.patient_id,\n" +
                "                                     f.visit_date,\n" +
                "                                     mid(max(concat(f.visit_date, f.breastfeeding)), 11)         as breastfeeding,\n" +
                "                                     mid(max(concat(f.visit_date, f.pregnancy_outcome)), 11)     as pregnant,\n" +
                "                                     mid(max(concat(f.visit_date, f.prep_contraindicatios)), 11) as prep_contraindication\n" +
                ...
                "                                 on f.patient_id = t.patient_id and f.visit_date = t.visit_date)p\n" +
                "               on e.patient_id = p.patient_id\n" +
                "      group by patient_id\n" +
                "      having (date_first_enrolled < date(:startDate) and fup_patient is not null)\n" +
                "          or (latest_enr_date between date(:startDate) and date(:endDate) and\n" +
                "              latest_enr_client_type = 'Re-enrollment(Re-activation)')\n" +
                "     )p;";
        SqlCohortDefinition cd = new SqlCohortDefinition();
        cd.setName("PrEP_CT");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.setDescription("Returned for PrEP follow-up or re-initiation");
        return cd;
     ...
     * PrEP_CT_HIV_STAT indicator
     * @return
     */
    public CohortDefinition prepCTByHIVStatus(String hivStatus) {
        ...
        String sqlQuery = "select p.patient_id\n" +
                ...
                "                                 on f.patient_id = t.patient_id and f.visit_date = t.visit_date)p\n" +
                "               on e.patient_id = p.patient_id\n" +
                "where p.final_test_result= "+hivStatus+"\n" +
                "      group by patient_id\n" +
                "      having (date_first_enrolled < date(:startDate) and fup_patient is not null)\n" +
                "          or (latest_enr_date between date(:startDate) and date(:endDate) and\n" +
                "              latest_enr_client_type = 'Re-enrollment(Re-activation)')\n" +
        ...
        SqlCohortDefinition cd = new SqlCohortDefinition();
        cd.setName("PrEP_CT_HIV_STAT");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.setDescription("Returned for PrEP follow-up or re-initiation by HIV status");
        return cd;
     ...
     * @return
     */
    public CohortDefinition prepCTKP(KPTypeDataDefinition kpType) {
        String sqlQuery = "select p.patient_id\n" +
                "from (select e.patient_id,\n" +
                "             e.first_enr_date         as date_first_enrolled,\n" +
                "             e.latest_enr_client_type as latest_enr_client_type,\n" +
                ...
                "                                 on f.patient_id = t.patient_id and f.visit_date = t.visit_date)p\n" +
                "               on e.patient_id = p.patient_id\n" +
                "      where e.kp_type = "+kpType.getKpTypeConcept()+"\n" +
                "      group by patient_id\n" +
                "      having (date_first_enrolled < date(:startDate) and fup_patient is not null)\n" +
                "          or (latest_enr_date between date(:startDate) and date(:endDate) and\n" +
                "              latest_enr_client_type = 'Re-enrollment(Re-activation)')\n" +
                "     )p;";
        SqlCohortDefinition cd = new SqlCohortDefinition();
        cd.setName("PrEP_CT_KP");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.setDescription("KPs Returned for PrEP follow-up or re-initiation");
        return cd;
    ...
    public CohortDefinition prepCTPregnant() {
        String sqlQuery = "select p.patient_id\n" +
                ...
                "     )p;";
        SqlCohortDefinition cd = new SqlCohortDefinition();
        cd.setName("PrEP_CT_PG");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.setDescription("Returned for PrEP follow-up or re-initiation while pregnant");
        return cd;
    ...
    public CohortDefinition prepCTBreastfeeding() {
        String sqlQuery = "select p.patient_id\n" +
                ...
                "     )p;";
        SqlCohortDefinition cd = new SqlCohortDefinition();
        cd.setName("PrEP_CT_BF");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.setDescription("Returned for PrEP follow-up or re-initiation while Breastfeeding");
        return cd;
    ...
    public CohortDefinition kpPrev(String kpType) {
        ...
        String sqlQuery = "select c.client_id from kenyaemr_etl.etl_contact c\n" +
                ...
                "                    inner join (select v.client_id, v.visit_date as clinical_visit_date,v.test_results from kenyaemr_etl.etl_clinical_visit v)v on c.client_id = v.client_id\n" +
                "                    left join (select t.patient_id,max(t.visit_date) as latest_hts_date,mid(max(concat(t.visit_date,t.final_test_result)),11) as hts_test_results from kenyaemr_etl.etl_hts_test t group by t.patient_id)t on c.client_id = t.patient_id\n" +
                "                    left join (select he.patient_id,max(he.visit_date) as latest_hiv_enr from kenyaemr_etl.etl_hiv_enrollment he group by he.patient_id)he on c.client_id = he.patient_id\n" +
                ...
                "                where((((v.clinical_visit_date between date(case MONTH(:startDate)\n" +
                "when (1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9) then replace('\"    + startOfYear\"', '0000', (YEAR(date_sub(:startDate, INTERVAL 1 YEAR))))\n" +
                "when 10 or 11 or 12 then replace('" + startOfYear + "', '0000', YEAR(:startDate))\n" +
                "    else null end) and date(:endDate)))\n" +
                "    or (p.first_peer_enc between (case MONTH(:startDate)\n" +
                "            when (1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9)\n" +
                "                    then replace('"    + startOfYear  +  "', '0000', (YEAR(date_sub(:startDate, INTERVAL 1 YEAR))))\n" +
                "        when (10 or 11 or 12)                then replace('\"    + startOfYear    \"', '0000', YEAR(:startDate))\n" +
                "            else null end) and date(:endDate)) and\n" +
                "    c.key_population_type = "+kpType+"\n" +
                "    and c.voided=0 and (t.latest_hts_date between (case MONTH(:startDate)\n" +
                "        when (1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9)\n" +
                "                then replace('" + startOfYear+"', '0000', (YEAR(date_sub(:startDate, INTERVAL 1 YEAR))))\n" +
                "        when (10 or 11 or 12)\n" +
                "                then replace('"    + startOfYear +"', '0000', YEAR(:startDate))\n" +
                "    else null end) and date(:endDate)))) group by c.client_id;";
        cd.setName("kpPrev");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
    ...
    public CohortDefinition kpPrevKnownPositive(String kpType) {
        ...
        String sqlQuery = "select c.client_id from kenyaemr_etl.etl_contact c\n" +
                ...
                "                    left join (select p.client_id, min(p.visit_date) as first_peer_enc from kenyaemr_etl.etl_peer_calendar p group by p.client_id)p on c.client_id = p.client_id\n" +
                "                where((((v.clinical_visit_date between date(case MONTH(:startDate)\n" +
                "when (1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9) then replace('" + startOfYear+"\n" +
                "    ', '0000', (YEAR(date_sub(:startDate, INTERVAL 1 YEAR))))\n" +
                "when 10 or 11 or 12 then replace('"+ startOfYear+"\n" +
                "', '0000', YEAR(:startDate))\n" +
                "else null end) and date(:endDate)))\n" +
                "or (p.first_peer_enc between (case MONTH(:startDate)\n" +
                "        when (1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9)\n" +
                "                then replace('" + startOfYear+"\n" +
                "    ', '0000', (YEAR(date_sub(:startDate, INTERVAL 1 YEAR))))\n" +
                "        when (10 or 11 or 12)\n" +
                "                then replace('" +startOfYear+"\n"+
                "', '0000', YEAR(:startDate))\n" +
                "        else null end) and date(:endDate)) and\n" +
                "c.key_population_type = "+kpType+"\n" +
                "and c.voided=0 and (he.latest_hiv_enr < e.enrolment_date or hiv_status_at_enrolment = 'Yes I tested positive' or v.test_results = 'Known Positive'))) group by c.client_id;";
        cd.setName("kpPrevKnownPositive");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.setDescription("kpPrevKnownPositive");

        return cd;
    }
    /**
     *  Number of patients newly tested for HIV (withing last 3 months)
     * NEWLY_TESTED
     */
    public CohortDefinition newlyTested() {

        SqlCohortDefinition cd = new SqlCohortDefinition();

        String sqlQuery = "select t.patient_id from kenyaemr_etl.etl_hts_test t group by t.patient_id having timestampdiff(MONTH,date(max(t.visit_date)),date(:endDate)) <= 3;";
        cd.setName("newlyTested");
        cd.setQuery(sqlQuery);
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.setDescription("newlyTested");
    ...
    public CohortDefinition kpPrevNewlyTested(String kpType) {
        CompositionCohortDefinition cd = new CompositionCohortDefinition();
        cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
        cd.addParameter(new Parameter("endDate", "End Date", Date.class));
        cd.addSearch("kpPrev",
                ReportUtils.map(kpPrev(kpType), "startDate=${startDate},endDate=${endDate}"));
        cd.addSearch("newlyTested", ReportUtils.map(newlyTested(), "startDate=${startDate},endDate=${endDate}"));
        cd.setCompositionString("kpPrev AND newlyTested");
        return cd;
    }
    /**
