...
public class CommonMetadata extends AbstractMetadataBundle {
    ...
    public static final String GP_EKYC_TOKEN_URL = "kenyaemr.ekyc.tokenUrl";
    public static final String GP_EKYC_REQUEST_URL = "kenyaemr.ekyc.requestUrl";
    public static final String GP_EKYC_CALLBACK_URL = "kenyaemr.ekyc.callbackUrl";
    public static final String GP_EKYC_MAX_ATTEMPTS = "kenyaemr.ekyc.maxAttempts";
    public static final String GP_EKYC_CLIENT_ID = "kenyaemr.ekyc.clientId";
    public static final String GP_EKYC_CLIENT_SECRET = "kenyaemr.ekyc.clientSecret";
	...
	public void install() {
        ...
        if(Context.getAdministrationService().getGlobalPropertyObject(CommonMetadata.GP_EKYC_TOKEN_URL) == null) {
            install(globalProperty(GP_EKYC_TOKEN_URL, "EKYC Authentication token url", ""));
        }
        if(Context.getAdministrationService().getGlobalPropertyObject(CommonMetadata.GP_EKYC_REQUEST_URL) == null) {
            install(globalProperty(GP_EKYC_REQUEST_URL, "EKYC Authentication request url", ""));
        }
        if(Context.getAdministrationService().getGlobalPropertyObject(CommonMetadata.GP_EKYC_CALLBACK_URL) == null) {
            install(globalProperty(GP_EKYC_CALLBACK_URL, "EKYC Authentication callback url", ""));
        }
        if(Context.getAdministrationService().getGlobalPropertyObject(CommonMetadata.GP_EKYC_MAX_ATTEMPTS) == null) {
            install(globalProperty(GP_EKYC_MAX_ATTEMPTS, "EKYC Authentication max attempts", ""));
        }
        if(Context.getAdministrationService().getGlobalPropertyObject(CommonMetadata.GP_EKYC_CLIENT_ID) == null) {
            install(globalProperty(GP_EKYC_CLIENT_ID, "EKYC Authentication client id", ""));
        }
        if(Context.getAdministrationService().getGlobalPropertyObject(CommonMetadata.GP_EKYC_CLIENT_SECRET) == null) {
            install(globalProperty(GP_EKYC_CLIENT_SECRET, "EKYC Authentication client secret", ""));
        }
        if(Context.getAdministrationService().getGlobalPropertyObject(CommonMetadata.GP_EKYC_EMBED_IFRAME_URL) == null) {
            install(globalProperty(GP_EKYC_EMBED_IFRAME_URL, "EKYC Authentication embed iframe url", ""));
        }
        if(Context.getAdministrationService().getGlobalPropertyObject(CommonMetadata.GP_EKYC_TOKEN_REFRESH_MARGIN_SECONDS) == null) {
            install(globalProperty(GP_EKYC_TOKEN_REFRESH_MARGIN_SECONDS, "EKYC Authentication token refresh margin seconds", ""));
        }
